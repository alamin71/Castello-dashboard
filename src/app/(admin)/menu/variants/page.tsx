...
const VARIANT_CATEGORIES: VariantCategory[] = [
  { id: "VC-34611", name: "Inch", assignedItems: 3, status: "active" },
  { id: "VC-34611", name: "Liter", assignedItems: 2, status: "active" },
  { id: "VC-34611", name: "Size", assignedItems: 5, status: "inactive" },
  { id: "VC-34611", name: "Weight", assignedItems: 3, status: "active" },
];

const VARIANT_ITEMS: VariantItem[] = [
  { id: "VI-34611", name: '15"', category: "Inch", status: "active" },
  { id: "VI-34612", name: '12"', category: "Inch", status: "active" },
  { id: "VI-34613", name: '9"', category: "Inch", status: "inactive" },
  { id: "VI-34614", name: "0.5L", category: "Liter", status: "inactive" },
  { id: "VI-34615", name: "1L", category: "Liter", status: "active" },
  { id: "VI-34616", name: "2L", category: "Liter", status: "active" },
];
      ...
      <div className="bg-[#1a1a1a] rounded-2xl w-full max-w-[500px] mx-4">
        ...
        <div className="px-6 pb-6 pt-2 border-t border-white/10">
          <div className="flex gap-3 mt-4">
            ...
            <button className="flex-1 py-3 rounded-xl bg-white text-black text-sm font-medium hover:bg-white/90 transition-colors">
              Save Category
            </button>
          </div>
        </div>
...
export default function VariantsPage() {
  ...
  const [categories, setCategories] = useState(VARIANT_CATEGORIES);
  const [items, setItems] = useState(VARIANT_ITEMS);
    ...
    <div className="p-6 min-h-screen">
      ...
      <div className="flex gap-2 mb-5">
        {(["categories", "items"] as Tab[]).map((t) => (
          <button
            key={t}
            onClick={() => { setTab(t); setSearch(""); setOpenMenu(null); }}
          ...
          {tab === "items" && (
            <div className="relative">
              ...
              >
                ...
                {VARIANT_CATEGORIES.map((c) => <option key={c.name} value={c.name}>{c.name}</option>)}
              </select>
              <ChevronDown size={14} className="absolute right-3 top-1/2 -translate-y-1/2 text-white/40 pointer-events-none" />
            </div>
          )}
                  ...
                  <tr key={idx} className="border-b border-white/4 hover:bg-white/2 transition-colors">
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-1 relative">
                        <button className="p-1.5 text-white/40 hover:text-white transition-colors rounded-lg hover:bg-white/5">
                          <Pencil size={15} />
                        </button>
                        <button onClick={() => setOpenMenu(openMenu === idx ? null : idx)} className="p-1.5 text-white/40 hover:text-white transition-colors rounded-lg hover:bg-white/5">
                          <MoreVertical size={15} />
                        </button>
                        {openMenu === idx && (
                          <div className="absolute right-0 top-8 z-20 bg-[#232323] border border-white/10 rounded-xl shadow-xl min-w-32.5 py-1">
                            <button onClick={() => toggleCatStatus(idx)} className={`flex items-center gap-2 w-full px-4 py-2.5 text-sm ${cat.status === "active" ? "text-red-400" : "text-emerald-400"} hover:bg-white/5 transition-colors`}>
                              <span className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${cat.status === "active" ? "border-red-400" : "border-emerald-400"}`} />
                  ...
                  <tr key={idx} className="border-b border-white/4 hover:bg-white/2 transition-colors">
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-1 relative">
                        ...
                        {openMenu === idx && (
                          <div className="absolute right-0 top-8 z-20 bg-[#232323] border border-white/10 rounded-xl shadow-xl min-w-32.5 py-1">
                            <button onClick={() => toggleItemStatus(idx)} className={`flex items-center gap-2 w-full px-4 py-2.5 text-sm ${item.status === "active" ? "text-red-400" : "text-emerald-400"} hover:bg-white/5 transition-colors`}>
                              <span className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${item.status === "active" ? "border-red-400" : "border-emerald-400"}`} />
