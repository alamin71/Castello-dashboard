...
import {
  Plus, Search, Pencil, MoreVertical, X, ChevronDown, Trash2, Ban, CircleCheck,
} from "lucide-react";
...
function CategoryModal({ existing, onClose }: { existing?: VariantCategory; onClose: () => void }) {
      ...
      <div className="bg-[#1a1a1a] rounded-2xl w-full max-w-125 mx-4">
        ...
        <div className="px-6 py-5">
          <div className="space-y-1.5">
            <label className="text-sm font-medium text-white"><span className="text-red-400">*</span> Variant Category</label>
            <input
...
// ── Add/Edit Item Modal ────────────────────────────────────────
function ItemModal({ existing, categories, onClose }: { existing?: VariantItem; categories: VariantCategory[]; onClose: () => void }) {
  const [name, setName] = useState(existing?.name ?? "");
    ...
    <div className="flex flex-col h-full p-6">
      ...
      <div className="shrink-0 flex gap-2 mb-5">
        {(["categories", "items"] as Tab[]).map((t) => (
          <button
            key={t}
            onClick={() => { setTab(t); setSearch(""); setStatusFilter("all"); setCategoryFilter(""); setOpenMenu(null); }}
