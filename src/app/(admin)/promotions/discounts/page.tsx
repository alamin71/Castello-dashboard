...
import {
  ...
  Pencil, Trash2, X, MoreVertical, Ban, CircleCheck,
} from "lucide-react";

import { useDiscounts } from "@/hooks/queries/useDiscounts";
import { useCreateDiscount } from "@/hooks/mutations/useCreateDiscount";
import { useUpdateDiscount } from "@/hooks/mutations/useUpdateDiscount";
import { useDeleteDiscount } from "@/hooks/mutations/useDeleteDiscount";
import { useCategories } from "@/hooks/queries/useCategories";
import { useProducts } from "@/hooks/queries/useProducts";

import { DiscountItem, CreateDiscountPayload } from "@/types/discount.types";
...
function DeleteConfirmModal({ discount, onClose }: { discount: DiscountItem; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70">
      <div className="bg-[#1a1a1a] border border-white/10 rounded-2xl w-full max-w-sm mx-4 p-6">
        ...
        <h3 className="text-base font-semibold text-white mb-1">Delete Discount</h3>
        <p className="text-sm text-white/50 mb-6">
          Are you sure you want to delete{" "}
          <span className="text-white font-medium">{discount.name}</span>? This action cannot be undone.
        </p>
        <div className="flex gap-3">
          ...
          <button
            onClick={() => deleteDiscount(discount._id, { onSuccess: onClose })}
            disabled={isPending}
          ...
          <div>
            <div className="flex items-start justify-between mb-1">
              <div>
                ...
                <p className="text-xs text-white/40 mt-0.5">You can add multiple products into this discount.</p>
              </div>
...
function DiscountModal({ initialData, onClose }: { initialData?: DiscountItem; onClose: () => void }) {
  ...
  const isValid = name.trim() && Number(discountValue) > 0 && startDate && expireDate;
  ...
  const handleSubmit = () => {
    ...
    if (isEdit) {
      updateDiscount({ id: initialData._id, payload }, { onSuccess: onClose });
    } else {
      createDiscount(payload, { onSuccess: onClose });
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70">
      <div className="bg-[#1a1a1a] rounded-2xl w-full max-w-xl mx-4 max-h-[92vh] overflow-y-auto border border-white/8">
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          <div className="space-y-1.5">
            <label className="text-sm font-medium text-white">
              <span className="text-[#e85d26] mr-0.5">*</span>Discount Offer Name
            </label>
            <input
              value={name}
        ...
        <div className="px-6 pb-6 flex gap-3">
          ...
          >
            {isPending ? "Saving…" : isEdit ? "Update Discount" : "Save Discount"}
          </button>
        </div>
      </div>
...
export default function DiscountsPage() {
  ...
  const [editDiscount, setEditDiscount] = useState<DiscountItem | null>(null);
  const [deleteDiscount, setDeleteDiscount] = useState<DiscountItem | null>(null);
  ...
  const { data, isLoading, isError } = useDiscounts(params);
  ...
  const toggleStatus = (discount: DiscountItem) => {
    updateDiscount({ id: discount._id, payload: { status: discount.status === "active" ? "inactive" : "active" } });
    closeMenu();
  };

  const TABLE_HEADERS = ["Create Date", "Discount Name", "Discount", "Eligible Date", "Expire Date", "Status", "Action"];
    ...
    <div className="flex flex-col h-full p-6">
          ...
          <table className="w-full min-w-[760px]">
            ...
            <tbody>
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={7} className="px-5 py-16 text-center text-sm text-red-400">
                ...
                discounts.map((discount) => (
                  <tr key={discount._id} className="border-b border-white/4 hover:bg-white/2 transition-colors">
                    <DateCell iso={discount.createdAt} withSeconds />
                    <td className="px-5 py-4">
                      <p className="text-sm text-white font-medium">{discount.name}</p>
                    </td>
                    <td className="px-5 py-4 text-sm text-white/70 whitespace-nowrap">
                      {discount.discountMethod === "percent"
                        ? `${discount.discountValue}%`
                        : `${discount.discountValue.toLocaleString()} kr.`}
                    </td>
                    <DateCell iso={discount.startDate} />
                    <DateCell iso={discount.expireDate} />
                    <td className="px-5 py-4">
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-1">
                        <button
                          onClick={() => { setEditDiscount(discount); setShowModal(true); }}
                        ...
                        <button
                          onClick={(e) => openMenuAt(discount._id, e)}
                          className="p-1.5 text-white/40 hover:text-white transition-colors rounded-lg hover:bg-white/5"
                          title="More options"
                        >
      ...
      {openMenu && menuPos && (
        <>
          ...
          >
            {(() => {
              const discount = discounts.find((d) => d._id === openMenu);
              ...
              return (
                <>
                  ...
                  >
                    {discount.status === "active" ? <Ban size={16} /> : <CircleCheck size={16} />}
                    {discount.status === "active" ? "Inactive" : "Active"}
                  </button>
                  <div className="mx-4 h-px bg-white/8" />
                  <button
                    onClick={() => { setDeleteDiscount(discount); closeMenu(); }}
      ...
      {showModal && (
        <DiscountModal
          initialData={editDiscount ?? undefined}
          onClose={() => { setShowModal(false); setEditDiscount(null); }}
        />
      )}
      {deleteDiscount && (
        <DeleteConfirmModal discount={deleteDiscount} onClose={() => setDeleteDiscount(null)} />
      )}
    </div>
  );
}
